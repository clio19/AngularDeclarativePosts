...
export class DeclarativePostService {
  posts$ = this.http
    .get<{ [id: string]: IPost }>(
      `https://angular-rxjs-64eec-default-rtdb.europe-west1.firebasedatabase.app/posts.json`
    )
    .pipe(
      map((posts) => {
        let postsData: IPost[] = [];
  ...
  updatePostToServer(post: IPost) {
    return this.http.patch<IPost>(
      `https://angular-rxjs-64eec-default-rtdb.europe-west1.firebasedatabase.app/posts/${post.id}.json`,
      post
    );
  }
